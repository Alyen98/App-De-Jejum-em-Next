...
export default function LoginPage() {
  ...
  const onSubmit = async (data: AuthFormData) => {
    try {
      ...
      // Aqui pode se tratado erros como 'auth/invalid-credential'
    }
  };
  ...
  return (
    <div className="flex min-h-screen items-center justify-center p-4">
      <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-md space-y-4 bg-white p-8 rounded-lg shadow-md">
        <h2 className="text-2xl font-bold text-center">Entrar no Sistema</h2>

        <div>
          <label className="block text-sm font-medium">E-mail</label>
          <input
            {...register("email")}
            className="w-full p-2 border rounded mt-1 outline-none focus:ring-2 focus:ring-blue-500 text-black"
          />
